...
def newObject(feature, name):
    ...
    # it is probably best to delete it if it exists, in order to start from a clean slate
    if (obj != None):
...
def hideAll():
    for o in fc.ActiveDocument.Objects:
        hide(o)
    return 
...
def newCylinder(name, radius, height, placement):
    cylinder = newObject("Part::Cylinder", name)
    cylinder.Label = name
...
def exportStl(objs, fname):
    # TODO: clarify where relative path goes to
    Mesh.export(objs, fname)
    return


def newHollowCylinder(name, innerRadius, outerRadius, height, placement):
    innerCylinder = newCylinder(name+"Inner", innerRadius, height, placement)
